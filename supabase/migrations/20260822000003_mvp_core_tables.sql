...
-- so gen_random_bytes() must be schema-qualified. We also build a URL-safe
-- token manually via translate() instead of encode(..., 'base64url'), since
-- 'base64url' is not yet a valid encode() format in any released Postgres
-- version (it landed in the still-unreleased PG19, past PG18's freeze).
