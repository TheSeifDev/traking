...
const routeContracts: Array<[string, string[]]> = [
  ...
  ["app/(dashboard)/organizations/[organizationId]/settings/page.tsx", ["authorizeOrganizationAdmin", "Organization settings"]],
  ["app/api/videos/route.ts", ["withDashboardAuth", "resolveSpaceForUser", "resolveMutationScopeForUser", "scope.spaceId"]],
  ["app/api/videos/[id]/route.ts", ["withDashboardAuth", "resolveSpaceForUser", "resolveSpaceAdminForUser", "access.space.id"]],
  ["app/api/videos/[id]/watch-link/route.ts", ["withDashboardAuth", "resolveMutationScopeForUser", "scope.spaceId"]],
...
const viewerActivityDashboard = source("src/components/dashboard/ViewerActivityDashboard.tsx");
const viewerActivityExportRoute = source("app/api/analytics/viewer-activity/export/route.ts");
...
const viewerAnalyticsPage = source("app/(dashboard)/analytics/viewers/[viewerId]/page.tsx");
const viewerAnalyticsDashboard = source("src/components/dashboard/ViewerAnalyticsDashboard.tsx");
const viewerVideoAnalyticsPage = source("app/(dashboard)/analytics/videos/[id]/viewers/[viewerId]/page.tsx");
const viewerVideoAnalyticsDashboard = source("src/components/dashboard/ViewerVideoAnalyticsDashboard.tsx");
...
assert(activeSpaceRoute.includes("authorizeOrganizationMember") && activeSpaceRoute.includes("setAllSpacesPreference"), "All Spaces selection is server-authorized per-user via organization membership");
...
assert(videosPage.includes("context.type === \"all\"") && videosPage.includes("spaceCanManage={user.role === \"owner\"}") && videosPage.includes("allowAllSpaces={user.role === \"owner\"}"), "Owner All Spaces video library exposes scoped creation while preserving virtual scope semantics");
assert(analyticsPage.includes("context.type === \"all\"") && analyticsPage.includes("organizationDataScope") && analyticsPage.includes("getViewerActivityAnalytics(scope") && analyticsPage.includes("defaultViewerActivityPeriod") && analyticsPage.includes("ViewerActivityDashboard"), "All Spaces analytics uses the complete authorized Viewer Activity organization aggregate");
assert(videoService.includes("getViewerActivityAnalytics") && videoService.includes("previousAnalyticsPeriod") && videoService.includes("filterActivitySessions") && videoService.includes("activityCompletionDistribution") && videoService.includes("sessions_over_time"), "Viewer Activity analytics calculates date-scoped filtered KPIs, trends, and completion distribution server-side");
assert(viewerActivityDashboard.includes("Unique viewers") && viewerActivityDashboard.includes("Measured watch time") && viewerActivityDashboard.includes("Avg watch time") && viewerActivityDashboard.includes("Avg completion") && viewerActivityDashboard.includes("90%+ completion rate") && viewerActivityDashboard.includes("Sparkline"), "Viewer Activity renders six real KPI cards with period trends");
assert(viewerActivityDashboard.includes("Search viewers, email, session, video, or provider") && viewerActivityDashboard.includes("minimum_sessions") && viewerActivityDashboard.includes("Clear") && viewerActivityDashboard.includes("Apply period"), "Viewer Activity exposes real search, minimum-session, clear, and date filters");
assert(viewerActivityDashboard.includes("Viewer sessions over time") && viewerActivityDashboard.includes("unique_viewers") && viewerActivityDashboard.includes("Completion distribution") && viewerActivityDashboard.includes("COMPLETION_COLORS") && viewerActivityDashboard.includes("measuredSessionCount"), "Viewer Activity renders real sessions-over-time and completion distribution visualizations");
assert(viewerActivityDashboard.includes("Viewer directory") && viewerActivityDashboard.includes("ViewerDesktopTable") && viewerActivityDashboard.includes("ViewerMobileCards") && viewerActivityDashboard.includes("View viewer"), "Viewer Activity renders identity-based responsive viewer directory navigation");
assert(viewerActivityExportRoute.includes("guardAuth") && viewerActivityExportRoute.includes("resolveActiveSpaceForUser") && viewerActivityExportRoute.includes("getViewerActivityAnalytics") && viewerActivityExportRoute.includes("text/csv"), "Viewer Activity export is scope-authorized and generated from calculated server data");
assert(watchLinksPage.includes("context.type === \"all\"") && watchLinksPage.includes("organizationDataScope") && watchLinksPage.includes("listVideos(scope)") && watchLinksPage.includes("organizationId={organization.id}") && watchLinksPage.includes("spaceCanManage={user.role === \"owner\"}"), "Owner All Spaces watch links remain organization-scoped and expose safe management");
...
assert(videoService.includes("getViewerAnalytics") && videoService.includes("getViewerVideoAnalytics") && videoService.includes("watch_links.video_id") && videoService.includes("unique_coverage_seconds") && videoService.includes("rewatch_count") && videoService.includes("videosById"), "viewer analytics service aggregates authorized viewer/video sessions from persisted relationships");
assert(viewerAnalyticsPage.includes("guardAuth") && viewerAnalyticsPage.includes("resolveActiveSpaceForUser") && viewerAnalyticsPage.includes("getViewerAnalytics") && viewerAnalyticsPage.includes("organizationDataScope"), "viewer-first analytics route is protected and scope-aware");
assert(viewerAnalyticsDashboard.includes("Videos watched by this viewer") && viewerAnalyticsDashboard.includes("Playback heatmap") && viewerAnalyticsDashboard.includes("View video analytics") && viewerAnalyticsDashboard.includes("No videos match your search") && viewerAnalyticsDashboard.includes("No sessions match your filters"), "viewer analytics dashboard provides independent video rows, tabs, filters, and honest empty states");
assert(viewerVideoAnalyticsPage.includes("getViewerVideoAnalytics") && viewerVideoAnalyticsPage.includes("resolveActiveSpaceForUser") && viewerVideoAnalyticsPage.includes("ViewerVideoAnalyticsDashboard"), "viewer/video analytics route uses a scoped dedicated service and existing authorization resolution");
assert(viewerVideoAnalyticsDashboard.includes("Watched coverage") && viewerVideoAnalyticsDashboard.includes("Playback position") && viewerVideoAnalyticsDashboard.includes("Event timeline") && viewerVideoAnalyticsDashboard.includes("Sessions for this video") && viewerVideoAnalyticsDashboard.includes("seek gaps") && viewerVideoAnalyticsDashboard.includes("provider-reported"), "viewer/video analytics exposes coverage, position chart, event timeline, and all-session history with telemetry honesty");
