...
async function runTests(): Promise<void> {
  ...
  assert(watchPlayer.includes("widget_referrer=${encodeURIComponent(window.location.origin)}") && watchPlayer.includes('iframe.referrerPolicy = "strict-origin-when-cross-origin"') && watchPlayer.includes("iframe.src = `https://www.youtube.com/embed/"), "YouTube IFrame API receives a valid origin referrer configuration before iframe load");
  ...
  assert(!videoDetailPage.includes(">Source</p>") && !videoDetailPage.includes(">{video.source_url}</p>"), "video detail never renders the raw provider source URL");
  assert(videoAnalyticsDashboard.includes("TrackingTooltip") && videoAnalyticsDashboard.includes("bg-[#11132d]/95") && videoAnalyticsDashboard.includes('cursor={{ fill: "#ffffff08"') && !videoAnalyticsDashboard.includes("contentStyle"), "tracking chart uses an explicit dark tooltip and subtle cursor without the default light flash");
  assert(videoAnalyticsDashboard.includes("buildSessionProgression") && videoAnalyticsDashboard.includes("heatmap.ranges") && videoAnalyticsDashboard.includes("Session playback progression") && videoAnalyticsDashboard.includes("furthest reliable position") && videoAnalyticsDashboard.includes("Session timeline"), "tracking visualization uses persisted watched ranges and distinguishes session progression from aggregate metrics");
  ...
  assert(analyticsService.includes("const hasPlaybackStart") && analyticsService.includes("supportsPlaybackMetrics(video.source_type) && hasPlaybackStart && telemetryEventCount > 0"), "analytics requires an actual play/resume event before marking a session measured");
  ...
  assert(workspaceAnalyticsDashboard.includes("analytics.viewer_sessions") && viewerAnalyticsPanel.includes("Top viewers") && viewerAnalyticsPanel.includes("Session only") && viewerAnalyticsPanel.includes("View timeline") && viewerAnalyticsPanel.includes("Status"), "analytics UI renders structured per-viewer sessions with honest provider scope");
