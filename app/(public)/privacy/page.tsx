      ...
      <section className="relative px-6 pb-20 pt-32 sm:pt-40 lg:px-10">
        ...
        <div className="relative mx-auto max-w-4xl">
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            {sections.map(({ title, icon: Icon, body }) => <article key={title} className="rounded-3xl border border-white/9 bg-white/[0.03] p-5 sm:p-6"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-violet-500/15 text-violet-200"><Icon size={19} /></div><h2 className="mt-6 text-lg font-semibold text-white">{title}</h2><p className="mt-3 text-sm leading-6 text-white/45">{body}</p></article>)}
          </div>
