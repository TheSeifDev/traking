...
function capability(sourceType: Parameters<typeof getProviderAdapter>[0]): string {
  const provider = getProviderAdapter(sourceType);
  return provider.capabilities.detailed_tracking
    ? `${provider.label} ${provider.playback_metrics_scope.replaceAll("_", " ")} telemetry`
    : `${provider.label} session-only measurement; provider playback callbacks unavailable`;
}
...
export default async function VideoDetailPage({ params, searchParams }: Props) {
  ...
  const canManage = resolution.context.type === "all"
    ? user.role === "owner"
    : Boolean(access?.is_platform_owner || access?.membership?.role === "admin");
  ...
  const youtubeId = provider.source_type === "youtube" ? getYouTubeId(video.source_url) : null;
