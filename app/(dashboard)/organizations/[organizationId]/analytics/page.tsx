...
export default async function OrganizationAnalyticsPage({ params }: { params: Promise<{ organizationId: string }> }) {
  ...
  // Organization aggregate analytics are an admin/owner surface. A viewer-level
  // org member must receive their own personal sessions, not the org totals.
  const canManageOrg = access.is_platform_owner || access.membership?.role === "admin";
  const organizationScope = access.is_platform_owner ? organizationDataScope(access.organization) : null;
  if (!canManageOrg) {
    const personalAnalytics = organizationScope
      ? await getWorkspaceAnalytics(organizationScope, user.id)
      : await viewerPersonalAnalytics(spaces, user.id);
    return <main className="min-h-full bg-[#08081f] px-4 py-6 text-white sm:px-6 lg:px-8 lg:py-8"><div className="mx-auto max-w-[1200px] space-y-7"><Link href={`/organizations/${organizationId}`} className="text-xs text-violet-300 hover:text-violet-200">← {access.organization.name}</Link><header className="border-b border-white/8 pb-7"><p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.25em] text-violet-300/70">Organization analytics</p><h1 className="mt-3 text-3xl font-semibold">{access.organization.name}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/45">Only sessions attributed to your authenticated TrackUp profile are included. Organization admins can access aggregate Organization analytics.</p></header><section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Your sessions" value={personalAnalytics.total_sessions} icon={Eye} /><Metric label="Your videos" value={new Set(personalAnalytics.viewer_sessions.map((session) => session.video_id)).size} icon={Layers3} /><Metric label="Your measured watch time" value={duration(personalAnalytics.total_measurable_watch_time_seconds)} icon={Clock3} /><Metric label="Provider state" value={personalAnalytics.playback_metrics_available ? "Measured" : "Not measured"} icon={Activity} /></section></div></main>;
  }
  const spaceAnalytics = organizationScope
    ? [await getWorkspaceAnalytics(organizationScope, undefined, undefined, false)]
    : await Promise.all(spaces.flatMap((space) => {
      const scope = spaceDataScope(space);
      return scope ? [getWorkspaceAnalytics(scope, undefined, undefined, false)] : [];
    }));
  const analytics = aggregate(spaceAnalytics);
  return (
    <main className="min-h-full bg-[#08081f] px-4 py-6 text-white sm:px-6 lg:px-8 lg:py-8">
      <div className="mx-auto max-w-[1200px] space-y-7">
        <Link href={`/organizations/${organizationId}`} className="text-xs text-violet-300 hover:text-violet-200">← {access.organization.name}</Link>
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Metric label="Unique viewers" value={analytics.unique_viewers} icon={UsersRound} />
          <Metric label="Sessions" value={analytics.total_sessions} icon={Layers3} />
          <Metric label="Measured watch time" value={duration(analytics.total_measurable_watch_time_seconds)} icon={Clock3} />
        </section>
        <section className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <article className="rounded-3xl border border-white/8 bg-white/[0.03] p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-[0.2em] text-white/30">Top videos</p>
                <h2 className="mt-2 text-lg font-semibold">Across this Organization</h2>
              </div>
              <BarChart3 size={18} className="text-violet-300" />
            </div>
            {analytics.top_videos_by_views.length === 0
              ? <p className="mt-6 rounded-2xl border border-dashed border-white/10 p-8 text-center text-sm text-white/35">No persisted analytics are available yet.</p>
              : (
                <div className="mt-5 space-y-3">
                  {analytics.top_videos_by_views.slice(0, 10).map((video) => (
                    <div key={video.video_id} className="flex items-center justify-between gap-3 rounded-xl border border-white/7 bg-black/10 px-3 py-3">
                      <span className="min-w-0 truncate text-sm text-white/75">{video.title}</span>
                      <span className="shrink-0 text-xs text-violet-200">{video.total_views} views</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <article className="rounded-3xl border border-white/8 bg-white/[0.03] p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-[0.2em] text-white/30">Measurement</p>
                <h2 className="mt-2 text-lg font-semibold">Provider-honest state</h2>
              </div>
              <Activity size={18} className="text-cyan-300" />
            </div>
            <div className="mt-5 space-y-3 text-sm text-white/55">
              <p>Measured sessions <span className="float-right text-white/80">{analytics.telemetry_health?.measured_sessions ?? 0}</span></p>
              <p>Missing telemetry <span className="float-right text-white/80">{analytics.telemetry_health?.missing_sessions ?? 0}</span></p>
              <p>Unsupported provider <span className="float-right text-white/80">{analytics.telemetry_health?.unsupported_sessions ?? 0}</span></p>
              <p>Average watch time <span className="float-right text-white/80">{duration(analytics.avg_watch_time_seconds)}</span></p>
              <p>Completion rate <span className="float-right text-white/80">{analytics.completion_rate === null ? "Not measured" : `${Math.round(analytics.completion_rate)}%`}</span></p>
            </div>
          </article>
        </section>
        <section className="rounded-3xl border border-white/8 bg-white/[0.03] p-5">
          <p className="text-[10px] uppercase tracking-[0.2em] text-white/30">Space context</p>
          <h2 className="mt-2 text-lg font-semibold">Organization Spaces</h2>
...
function emptyPersonalAnalytics(): WorkspaceAnalytics {
  return {
    total_videos: 0,
    total_views: 0,
    total_sessions: 0,
...
async function viewerPersonalAnalytics(spaces: Space[] | null, viewerProfileId: string): Promise<WorkspaceAnalytics> {
  if (!spaces || spaces.length === 0) return emptyPersonalAnalytics();
  for (const space of spaces) {
    const scope = spaceDataScope(space);
    if (!scope) continue;
    return await getWorkspaceAnalytics(scope, viewerProfileId);
  }
