...
export default async function VideoAnalyticsPage({ params, searchParams }: Props) {
  ...
  if (!scope || !canManage) return <EmptyAnalytics title="Space admin access required" body="Per-video aggregate analytics are restricted to the platform owner and active Space admins." />;
  const [video, analytics] = await Promise.all([
    getVideo(id, scope),
    getVideoAnalytics(id, scope),
  ]);
  if (!video || !analytics) return <EmptyAnalytics title="Video analytics unavailable" body="The video may not belong to this authorized Organization or Space, or the analytics records could not be read." />;
  ...
  const telemetryNote = analytics.playback_metrics_scope === "session_only"
    ? "Provider lifecycle only"
    : measured > 0
      ? `${measured} measured session${measured === 1 ? "" : "s"}`
      : "Playback telemetry not recorded";
  const telemetryNoteClass = analytics.playback_metrics_scope === "session_only"
