...
export default async function SessionAnalyticsPage({ params, searchParams }: Props) {
  ...
  if (requestedSpaceId) {
    ...
    if (!spaces[0]) return <EmptyAnalytics title="No accessible Space" body="Join a Space before opening session analytics." />;
    try {
  ...
  if (!canManage) return <EmptyAnalytics title="Space admin access required" body="Session analytics are restricted to the platform owner and active Space admins." />;
  if (!access.space.clickup_workspace_id) return <EmptyAnalytics title="No ClickUp Workspace connected" body="Connect ClickUp before opening session analytics." />;
  const session = await getVideoSessionAnalytics(id, access.space.clickup_workspace_id, sessionId, access.space.id);
  if (!session) return <EmptyAnalytics title="Session analytics unavailable" body="This session is not present in the selected video or is outside your Space." />;
