...
function spaceRole(member: SpaceMemberView): string {
  return member.role === "admin" ? "SPACE ADMIN" : "SPACE MEMBER";
}
...
function errorCopy(value: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    organization_mismatch: "The profile must belong to this Organization before Space access can be assigned.",
  };
    ...
    <div className="min-h-full bg-[#08081f] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <div className="mx-auto max-w-[1200px] space-y-7">
        <header className="flex flex-col gap-4 border-b border-white/8 pb-7 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <Link href={`/spaces/${spaceId}`} className="inline-flex items-center gap-1.5 text-xs text-white/38 transition hover:text-violet-200"><ArrowLeft size={14} />Back to Space</Link>
            <p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.25em] text-violet-300/70">Space members</p>
            <h1 className="mt-3 text-3xl font-semibold tracking-[-0.035em] text-white">Space access</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/45">Manage access to this real Space only. Organization roles remain the source of truth for Organization authority and are never changed by Space membership.</p>
          </div>
        ...
        <section className="rounded-3xl border border-white/9 bg-white/[0.035] p-5 sm:p-6">
          ...
          <p className="mt-1 text-xs leading-5 text-white/35">Only active profiles already belonging to this Organization can be assigned to this Space. Space access does not grant Organization Admin or Owner authority.</p>
