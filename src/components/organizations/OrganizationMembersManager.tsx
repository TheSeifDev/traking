...
export default function OrganizationMembersManager({ organizationId, organizationName, currentUserId, initialMembers }: {
  ...
  const visibleMembers = normalizedMemberFilter.length === 0 ? members : members.filter((member) => {
    const searchable = `${displayName(member)} ${member.profile.email} ${member.profile.clickup_user_id ?? ""}`.toLowerCase();
    return searchable.includes(normalizedMemberFilter);
  });
