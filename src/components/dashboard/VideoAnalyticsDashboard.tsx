...
export default function VideoAnalyticsDashboard({ video, analytics }: VideoAnalyticsDashboardProps) {
  ...
  const measuredSessions = analytics.viewer_sessions.filter((session) => session.watch_time_seconds !== null);
  const progressSessions = analytics.viewer_sessions.filter((session) => measuredProgress(session) !== null);
  ...
  const stats = [
    ...
    { label: "Unique viewers", value: analytics.unique_viewers.toLocaleString(), note: "Hashed identities", icon: Users, color: "text-blue-300" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-linear-to-br from-violet-500/12 via-white/[0.03] to-blue-500/8 p-6 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 gap-4">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.18em] text-white/40"><span className="rounded-full border border-white/10 px-2 py-1 capitalize">{providerLabel(video.source_type)}</span><span className="text-violet-200/70">{scopeLabel(analytics.playback_metrics_scope)}</span></div>
      ...
      {section === "viewers" ? <ViewerAnalyticsPanel sessions={analytics.viewer_sessions} title="Viewer and session details" description="Every row is a real session. Viewer identity is a one-way hash; playback fields are available only for native HTML5 or YouTube IFrame API telemetry." /> : section === "activity" ? <ActivityTimeline sessions={analytics.viewer_sessions} /> : <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="space-y-6"><div className="rounded-3xl border border-white/8 bg-white/[0.03] p-5"><h2 className="text-base font-semibold text-white">Measurement scope</h2><div className="mt-5 space-y-4"><div className="flex items-center justify-between text-sm"><span className="text-white/45">Provider</span><span className="capitalize text-white/80">{providerLabel(video.source_type)}</span></div><div className="flex items-center justify-between text-sm"><span className="text-white/45">Playback source</span><span className="text-white/80">{scopeLabel(analytics.playback_metrics_scope)}</span></div><div className="flex items-center justify-between text-sm"><span className="text-white/45">Measured sessions</span><span className="text-white/80">{measuredSessions.length} / {analytics.total_sessions}</span></div><div className="flex items-center justify-between text-sm"><span className="text-white/45">Avg watch time</span><span className="text-white/80">{formatDuration(analytics.avg_watch_time_seconds)}</span></div></div></div><div className="rounded-3xl border border-amber-300/15 bg-amber-300/5 p-5"><h2 className="text-sm font-semibold text-amber-100">Coverage and heatmap</h2><p className="mt-2 text-xs leading-5 text-amber-100/55">Not measured. Stored events provide point-in-time playback evidence, but do not yet reconstruct continuous watched ranges reliably. TrackUp intentionally does not render a fabricated heatmap.</p></div></div>
      </div>}
    </div>
  );
}
