    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-linear-to-br from-violet-500/12 via-white/[0.03] to-blue-500/8 p-6 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 gap-4">
          <div className="hidden h-20 w-32 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] sm:block" style={thumbnailUrl ? { backgroundImage: `url(${thumbnailUrl})`, backgroundPosition: "center", backgroundSize: "cover" } : undefined} aria-label={thumbnailUrl ? "Provider video thumbnail" : "No preview available"}>
            {!thumbnailUrl && <VideoIconPlaceholder />}
          </div>
      ...
      {section === "viewers" ? <ViewerAnalyticsPanel sessions={analytics.viewer_sessions} title="Viewer and session details" description="Every row is a real session. New sessions may show the authenticated profile identity; legacy sessions remain hashed or anonymous. Playback fields are available only for providers with a registered telemetry adapter (native HTML5, YouTube IFrame API, or Vimeo Player SDK)." /> : section === "activity" ? <ActivityTimeline sessions={analytics.viewer_sessions} /> : <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
