...
function SessionTableRow({ session, video, href, viewerHref }: { session: ViewerSessionAnalytics; video?: Video; href: string; viewerHref: string | null }) {
  ...
  return <tr className="align-middle transition hover:bg-white/[0.025]"><td className="px-4 py-3"><div className="flex min-w-[210px] items-center gap-2.5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-violet-400/12 text-[10px] font-semibold text-violet-100">{initials(session)}</div><div className="min-w-0"><p className="max-w-[180px] truncate text-sm text-white/80">{viewerLabel(session)}</p><p className="max-w-[180px] truncate text-[11px] text-white/35">{viewerEmail(session)}</p><p className="text-[10px] text-white/25">#{session.session_number} · {session.session_id.slice(0, 8)}</p></div></div></td><td className="px-4 py-3"><div className="flex w-[210px] items-center gap-2">{thumbnail ? <img src={thumbnail} alt="" className="h-8 w-12 rounded-md object-cover" /> : <div className="flex h-8 w-12 items-center justify-center rounded-md bg-white/[0.05]"><VideoIcon size={13} className="text-white/25" /></div>}<div className="min-w-0"><p className="truncate text-xs text-white/70">{session.video_title}</p><p className="mt-0.5 text-[10px] capitalize text-white/30">{getProviderLabel(session.source_type)}</p></div></div></td><td className="whitespace-nowrap px-4 py-3 text-xs text-white/55"><p>{formatDate(session.started_at)}</p><p className="mt-1 text-[10px] text-white/30">Last {formatDate(session.last_activity_at)}</p></td><td className="whitespace-nowrap px-4 py-3 text-xs text-white/60"><p>{formatDuration(session.watch_time_seconds)}</p><p className="mt-1 text-[10px] text-white/30">Visit {formatDuration(sessionDuration(session))}</p></td><td className="px-4 py-3 text-xs text-white/60">{session.completion_percentage === null ? "Not measured" : `${Math.round(session.completion_percentage)}%`}<p className="mt-1 text-[10px] text-white/30">Last {formatPosition(session.last_position)}</p></td><td className="px-4 py-3"><span className={`inline-flex rounded-full border px-2 py-1 text-[10px] ${statusClass(session)}`}>{statusLabel(session)}</span><p className="mt-1 text-[10px] text-white/30">{session.playback_events.length} events</p></td><td className="px-4 py-3"><div className="flex flex-col items-start gap-1.5"><Link href={href} className="whitespace-nowrap text-xs font-medium text-violet-200 hover:text-violet-100">View timeline →</Link>{viewerHref && <Link href={viewerHref} className="whitespace-nowrap text-[10px] text-white/40 hover:text-white">View viewer</Link>}</div></td></tr>;
}
...
function SessionCard({ session, video, href, viewerHref }: { session: ViewerSessionAnalytics; video?: Video; href: string; viewerHref: string | null }) {
  const thumbnail = video?.source_url ? getProviderAdapter(video.source_type).thumbnail_url(video.source_url) : null;
  return <article className="rounded-2xl border border-white/8 bg-white/[0.025] p-4"><div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-violet-400/12 text-[10px] font-semibold text-violet-100">{initials(session)}</div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-medium text-white/85">{viewerLabel(session)}</p><p className="truncate text-xs text-white/35">{viewerEmail(session)}</p></div><span className={`rounded-full border px-2 py-1 text-[10px] ${statusClass(session)}`}>{statusLabel(session)}</span></div><p className="mt-3 text-[10px] text-white/25">Session {session.session_id.slice(0, 12)} · #{session.session_number} · {formatDate(session.started_at)}</p></div></div><div className="mt-4 flex items-center gap-3 rounded-xl border border-white/7 bg-black/10 p-2.5">{thumbnail ? <img src={thumbnail} alt="" className="h-10 w-16 rounded-lg object-cover" /> : <div className="flex h-10 w-16 items-center justify-center rounded-lg bg-white/[0.05]"><VideoIcon size={14} className="text-white/25" /></div>}<div className="min-w-0"><p className="truncate text-xs font-medium text-white/75">{session.video_title}</p><p className="mt-1 text-[10px] capitalize text-white/35">{getProviderLabel(session.source_type)} · {scopeLabel(session.playback_metrics_scope)}</p></div></div><div className="mt-4 grid grid-cols-2 gap-2"><Detail label="Watch time" value={formatDuration(session.watch_time_seconds)} /><Detail label="Visit duration" value={formatDuration(sessionDuration(session))} /><Detail label="Completion" value={session.completion_percentage === null ? "Not measured" : `${Math.round(session.completion_percentage)}%`} /><Detail label="Events" value={String(session.playback_events.length)} /></div><div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-white/7 pt-3"><span className="text-[11px] text-white/35">Last position: <strong className="font-medium text-white/60">{formatPosition(session.last_position)}</strong></span><div className="flex gap-3"><Link href={href} className="text-xs font-medium text-violet-200 hover:text-violet-100">View timeline →</Link>{viewerHref && <Link href={viewerHref} className="text-xs text-white/45 hover:text-white">Viewer</Link>}</div></div></article>;
}
