  ...
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.09),transparent_35%)] p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-7xl space-y-7 lg:space-y-8">
        <header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-violet-300/70">Team access</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">Team members</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-white/45">Invite teammates by email, activate their assigned ClickUp role after same-email authentication, and see server-recorded presence. Profiles remain global in the current architecture; workspace memberships are intentionally unchanged.</p></div><button onClick={() => void loadMembers()} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm font-medium text-white/65 transition hover:border-white/20 hover:bg-white/[0.08] hover:text-white disabled:opacity-50"><RefreshCw size={15} className={loading ? "animate-spin" : ""} />Refresh directory</button></header>
