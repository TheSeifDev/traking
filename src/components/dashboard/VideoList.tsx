...
export default function VideoList({ role, spaceId = null, organizationId = null, spaceCanManage, scopeOptions = [], allowAllSpaces = false }: VideoListProps) {
  const canManage = spaceCanManage ?? (role === "owner" || role === "admin");
  const encodedSpaceId = spaceId ? encodeURIComponent(spaceId) : "";
  const encodedOrganizationId = organizationId ? encodeURIComponent(organizationId) : "";
  const scopeQuery = encodedSpaceId ? `?space_id=${encodedSpaceId}` : encodedOrganizationId ? `?organization_id=${encodedOrganizationId}` : "";
  const scopedPath = (path: string) => `${path}${encodedSpaceId ? `${path.includes("?") ? "&" : "?"}space_id=${encodedSpaceId}` : encodedOrganizationId ? `${path.includes("?") ? "&" : "?"}organization_id=${encodedOrganizationId}` : ""}`;
  ...
  const renderCreateDialog = () => <CreateVideoDialog onCreated={fetchVideos} organizationId={organizationId} initialSpaceId={spaceId} scopeOptions={scopeOptions} allowAllSpaces={allowAllSpaces} />;

  return (
    <div className="min-h-full bg-[#08081f] px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
      <div className="mx-auto max-w-[1440px] space-y-7">
        <header className="flex flex-col gap-5 border-b border-white/8 pb-6 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-violet-300/70">Library</p><h1 className="mt-3 text-3xl font-semibold tracking-[-0.035em] text-white sm:text-4xl">Videos</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/45">{manageMessage}</p></div>{canManage && renderCreateDialog()}</header>
...
function EmptyState({ hasFilters, canManage, createAction, onClear }: { hasFilters: boolean; canManage: boolean; createAction?: React.ReactNode; onClear: () => void }) {
  return <div className="flex min-h-80 flex-col items-center justify-center rounded-3xl border border-dashed border-white/12 bg-white/[0.018] px-6 py-12 text-center"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-violet-400/10 text-violet-200"><FileVideo size={25} /></div><p className="mt-6 text-[10px] font-semibold uppercase tracking-[0.24em] text-violet-300/60">Video library</p><h2 className="mt-2 text-xl font-semibold text-white">{hasFilters ? "No videos match these filters" : canManage ? "No videos yet" : "Your videos will appear here"}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-white/40">{hasFilters ? "Try a different provider, viewer-link status, search term, or sort view." : canManage ? "Add your first video to create an internal TrackUp viewing experience and shareable viewer link." : "No videos have been shared with this workspace yet."}</p>{hasFilters ? <button onClick={onClear} className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-violet-300 hover:text-violet-200"><X size={15} />Clear filters</button> : canManage && createAction ? <div className="mt-6">{createAction}</div> : null}</div>;
}

function VideoCard({ video, organizationId, spaceId, canManage, currentTime, sharing, revoking, deleting, onShare, onOpenViewer, onRevoke, onDelete }: { video: VideoType; organizationId?: string | null; spaceId?: string | null; canManage: boolean; currentTime: number; sharing: boolean; revoking: boolean; deleting: boolean; onShare: () => void; onOpenViewer: () => void; onRevoke: () => void; onDelete: () => void }) {
