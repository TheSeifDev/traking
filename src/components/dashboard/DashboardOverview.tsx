...
export default function DashboardOverview({ user, analytics, videos, error = null, spaceId, canManage }: DashboardOverviewProps) {
  const [dateRange, setDateRange] = useState<DateRange>("30");
  const [now] = useState(() => Date.now());
  const firstName = user.name?.trim().split(/\s+/)[0] ?? user.email.split("@")[0];
  const scoped = (path: string) => `${path}${path.includes("?") ? "&" : "?"}space_id=${encodeURIComponent(spaceId)}`;
