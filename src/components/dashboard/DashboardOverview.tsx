...
export default function DashboardOverview({ user, analytics, videos, error = null }: DashboardOverviewProps) {
  ...
  const topVideos = useMemo(() => videos.map((video) => {
    ...
    return {
      ...
      viewers: new Set(sessions.map((session) => session.viewer_identifier ?? session.session_id)).size,
      ...
      <div className="mx-auto max-w-[1440px] space-y-8">
        <header className="flex flex-col gap-5 border-b border-white/8 pb-7 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.26em] text-violet-300/70"><span className="h-1.5 w-1.5 rounded-full bg-violet-300" />{analytics.total_videos > 0 ? "Workspace overview" : "Workspace setup"}</div>
          ...
          <div className="flex flex-wrap gap-2">
            {canManage && <Link href="/videos" className="inline-flex items-center gap-2 rounded-xl bg-violet-500 px-4 py-2.5 text-sm font-semibold text-white shadow-[0_8px_28px_rgba(124,58,237,0.28)] transition hover:bg-violet-400 active:scale-[0.98]"><Plus size={16} />Add video</Link>}
            <Link href="/analytics" className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm font-medium text-white/70 transition hover:border-white/20 hover:bg-white/[0.08] hover:text-white active:scale-[0.98]"><BarChart3 size={16} />View analytics</Link>
          </div>
        </header>
...
function ActivityRow({ session, now }: { session: ViewerSessionAnalytics; now: number }) {
  ...
  return <div className="flex min-w-0 items-start gap-3 py-3 first:pt-0 last:pb-0"><div className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${event ? "bg-violet-400/10 text-violet-200" : "bg-white/[0.05] text-white/35"}`}>{event ? <PlayCircle size={14} /> : <Eye size={14} />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm text-white/78"><span className="font-medium text-white">Viewer {session.viewer_identifier ?? "unidentified"}</span> <span className="text-white/38">{action.toLowerCase()}</span></p><p className="mt-1 truncate text-[11px] text-white/35">{session.video_title} · {formatProvider(session.source_type)} · Session {session.session_number}</p></div><span className="shrink-0 text-[10px] text-white/32">{formatRelative(session.last_activity_at, now)}</span></div>;
}
